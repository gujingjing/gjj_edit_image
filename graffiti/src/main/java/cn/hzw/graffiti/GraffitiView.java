...
import cn.hzw.graffiti.edit_image.PixelateUtil;
import cn.hzw.graffiti.mosaic.MosaicUtil;
...
public class GraffitiView extends View {
    ...
    private BitmapShader mBitmapShaderMosaic; //用于马赛克
    private Bitmap mosaicBitmap;//马赛克图片
    ...
    private void resetPaint(Pen pen, Paint paint, Matrix matrix, GraffitiColor color, int rotateDegree) {
        ...
        switch (pen) { // 设置画笔
            ...
            case ERASER:
                ...
                break;
            case MOSAIC://马赛克
                mBitmapShaderMosaic.setLocalMatrix(matrix);
...
//                paint.setPathEffect(new CornerPathEffect(10));
                paint.setStrokeWidth(Util.dp2px(getContext(), 20));

                paint.setShader(this.mBitmapShaderMosaic);
                break;
        }
    }
...
//            throw new NullPointerException("Selected item is null!");
    ...
    public void resetBitmap(Bitmap bitmap) {
        ...
        setBG();
        mCopyLocation.updateLocation(toX(mOriginalWidth / 2), toY(mOriginalHeight / 2));
        if (!mReady) {
            mGraffitiListener.onReady();
            mReady = true;
        }
